...
void drawImage(unsigned char x, unsigned char y, unsigned char sizex, unsigned char sizey, const unsigned char img[], unsigned char invert) {
  ...
  if (((x + sizex) > SSD1306_WIDTH) || ((y + sizey) > SSD1306_HEIGHT) ||
      (sizex == 0) || (sizey == 0)) return;
  ...
  p1 = (y + sizey - 1) / 8;      // last page index
  n = y % 8;                  // offset form begin of page

  n1 = (y + sizey) % 8;
  if (n1) n1 = 8 - n1;

  j = 0;                      // bytes counter [0..t], or [0..(t+sizex)]
  dataBuffer = malloc(sizex + 1);       // allocate memory for the buf
  ...
  for (p=p0; p<(p1+1); p++) {
    ...
    for (i=x; i<(x+sizex); i++) {
      if (p == p0) {
        ...
        b = (img[j - sizex] >> n1) & 0xFF;
      } else {
        b = ((img[j - sizex] >> (8 - n)) & 0xFF) | ((img[j] << n) & 0xFF);
      };
      if (invert)
          dataBuffer[i - x + 1] = b;
      else
    ...
    sendData(dataBuffer, sizex + 1); // send the buf to display
  }
  free(dataBuffer);
}
//******************************************************************************************************************************************
